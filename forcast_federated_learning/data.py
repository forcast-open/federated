...
def random_clients_data(data_len, n_clients, seed=0):
	'''
	...
	'''
	np.random.seed(seed)
	rand_dist = np.random.randint(low=1, high=2*(data_len/n_clients), size=n_clients)
...
def random_non_iid_split(dataset, n_clients=10, classes_per_client=2, shuffle=True, verbose=False, seed=0):
	'''
	...
	### client distribution ####
	data_per_client = random_clients_data(len(data), n_clients, seed=seed)
